...
function LdapAuth(opts) {
  ...
  if (opts.cache) {
    ...
    this.userCache = cached('users', {
      backend: { type: 'memcached', hosts: '127.0.0.1:11211', poolSize: 15 },
      expire: 300
    });
...
LdapAuth.prototype.authenticate = function(username, password, callback) {
  ...
  if (self.opts.cache) {
    ...
    self.userCache.get(username, function(err, data) {
      if (err) {
        self.log && self.log.trace('ldap authenticate: error checking cache %s', err);
      } else {
          ...
          self._findUser(username, function(findErr, user) {
            ...
            self._userClient.bind(user[self.opts.bindProperty], password, function(bindErr) {
              ...
              self._getGroups(user, function(groupErr, userWithGroups) {
                ...
                if (self.opts.cache) {
                  bcrypt.hash(password, self._salt, function(passwordErr, hash) {
                    if (passwordErr) {
                      self.log && self.log.trace('ldap authenticate: bcrypt error, not caching %s', passwordErr);
                    } else {
